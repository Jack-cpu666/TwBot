...
import shutil  # To find the Chrome executable
...
if not CHROME_EXECUTABLE_PATH:
    raise RuntimeError(
        "FATAL ERROR: Could not find 'google-chrome-stable' in your system's PATH. "
        "Please make sure you have installed Google Chrome correctly."
    )
...
def stream_screenshots(sid):
    ...
    if not session or not session.get('driver'): return
...
def handle_start_browser(data):
    ...
    try:
        print(f"[{sid}] Starting UNDETECTED browser with size {width}x{height} at URL: {url}")
        
        options = uc.ChromeOptions()
        
        # --- THE CRUCIAL FIX IS HERE ---
        # This tells Chrome to run in the background without a visible window.
        # This is ESSENTIAL for the application to work as intended.
        options.add_argument("--headless=new")
        
        # Other essential options for running in a server environment
        options.add_argument("--no-sandbox")
        options.add_argument("--disable-dev-shm-usage")
        options.add_argument(f"--window-size={width},{height}")
        
        # Use the auto-detected path we found when the script started.
        driver = uc.Chrome(
            options=options,
    ...
    try:
        if event_type == 'click':
            ...
            try:
                ...
                print(f"Could not find active element to send keys to: {e}")
    except Exception as e:
